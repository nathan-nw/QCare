...
const generateMockPatient = (overrides = {}) => {
  const triageCategory =
    overrides.triageCategory || generateMockTriageCategory();
  const timeElapsed =
    overrides.timeElapsed || generateMockWaitTime(triageCategory);
  const arrivalTime =
    overrides.arrivalTime || new Date(Date.now() - timeElapsed * 60000);
